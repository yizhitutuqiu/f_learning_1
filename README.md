...
**至少 1 条命令**即可跑齐论文对比实验（4 个数据集 × FedAvg + FedProx，共 8 组）：
...
- 最后调用 `summarize_results.py` 与 `plot_results.py`，在 `output/` 下生成汇总表与图表
...
- **训练/评估日志**
  - `logs/<name>.jsonl`：每行一个 round，字段 `round`, `accuracy`, `loss`
  - `logs/<name>.csv`：表头 `round,accuracy,loss`，便于做表或后续画图
  - `logs/<name>.txt`：与 FedProx 原版 plot 脚本兼容的文本日志
- **每次实验的配置文件快照**
  - `output/<name>_config.json`：当次运行的全部命令行参数与解析后的 `dataset_resolved`，保证可追溯
- **结果与曲线数据**
  - `output/<name>_metrics.json`：含 `final_accuracy`、`final_loss` 及每轮 `history`
...
├── run_all_synthetic.sh     # 一键复现 8 组实验
...
上述设置满足 mini-project 对 **README（可复现说明）、实验日志与原始输出、图表、关键超参数与随机种子** 的要求；一键复现命令为 **`bash run_all_synthetic.sh`**。
